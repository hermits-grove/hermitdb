...
fn test_iter() {
    ...
    assert_eq!(
        items.get(&("x".into(), Kind::Reg))
            .and_then(|e| e.val.clone().to_reg().ok())
            .map(|r| r.read().val),
